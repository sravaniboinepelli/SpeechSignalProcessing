clear, clc, close all
% load an audio file
%[x, fs] = audioread('common_voice_pa-IN_21722988.wav');

%modified_group_delay_feature(0.4, 0.9, 12 ,0.010);

%function [grp_phase, cep, ts] = modified_group_delay_feature(rho, gamma, num_coeff, frame_shift)
...
%[frames, ts] = v_enframe(speech, hamming(frame_length), frame_shift);
